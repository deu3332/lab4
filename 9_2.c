...
g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(click_symbol),
(gpointer)'-');
gtk_widget_show(button);
...
button = gtk_button_new_with_label("=");
g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(submit),
NULL);
...
{
if(second_number == -1 && first_number == -1 && has_press_number ==
0)
{}
else if(second_number == -1 && first_number == -1 &&
has_press_number == 1)
{
